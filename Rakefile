...
def write_file(content, title, directory, filename)
  ...
  parsed_content = "#{parsed_content.sub("date:", "date: #{Time.now.strftime '%Y-%m-%d %H:%M:%S'}")}"
  parsed_content = "#{parsed_content.sub("uid:", "uid: #{SecureRandom.base64 32}")}"
...
desc "RSync the site with NearlyFreeSpeech"
task :sync do
